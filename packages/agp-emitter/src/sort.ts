/**
 * ═══════════════════════════════════════════════════════════
 *  @arch-engine/agp-emitter — Record stream sort
 * ═══════════════════════════════════════════════════════════
 *
 *  Per spec §10.4 records.ndjson is sorted by the lexicographic
 *  tuple:
 *
...
export function sortRecords(records: ReadonlyArray<AgpRecord>): AgpRecord[] {
  return [...records].sort(compare);
}

function compare(a: AgpRecord, b: AgpRecord): number {
...
function primaryKey(r: AgpRecord): string {
  ...
  switch (r.family) {
    ...
    case 'diagnostic': {
      ...
      // severity DESC → use 9-rank so larger rank sorts first as string
    ...
    case 'drift': {
      ...
      // Sub-key: pick a stable representation per drift kind so two
      // drift records of the same kind against the same anchors do
