  ...
  return {
    ...
    readdirSync: vi.fn((p) => {
      if (typeof p === 'string' && p.includes('.arch-engine/policies')) {
        return ['governance.policy.json'];
      }
      return actual.readdirSync(p as any);
    }),
    readFileSync: vi.fn((p, enc) => {
      if (typeof p === 'string' && p.endsWith('governance.policy.json')) {
        return JSON.stringify({
          policyPackId: 'governance-pack',
          description: 'desc',
          category: 'cat',
...
describe('Phase 14C Policy-Pack Governance Enforcement', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  test('blocks execution if required dataset governance surface is missing', async () => {
    // A dataset that DOES NOT have required_mutation_class
    const dataset = {
