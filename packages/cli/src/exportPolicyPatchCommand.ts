...
export async function exportPolicyPatchCommand(args: string[]): Promise<void> {
    let topologyPath = 'topology-export.json';
    const packDirs: string[] = [];
    let jsonFlag = false;
    let markdownFlag = false;
    ...
    if (markdownFlag) {
        console.log(exportResult.markdownSummary);
        return;
    }

    console.log(exportResult.machineReadableSummary);
}
