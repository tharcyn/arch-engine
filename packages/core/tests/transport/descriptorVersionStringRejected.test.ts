...
describe('Phase 8.8: Descriptor Version String Rejected', () => {
  ...
  it('Test 1: Rejects string versions for remote matrix descriptors', () => {
    const stringMatrix = JSON.parse(JSON.stringify(baseRemoteMatrix));
    stringMatrix.compositionRuntimeCapabilityDescriptor.version = '5.0-alpha';
    
    expect(() => assertCapabilityDescriptorMatrixParity(stringMatrix))
