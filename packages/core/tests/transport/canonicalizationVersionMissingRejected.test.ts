...
describe('Phase 8.8: Canonicalization Version Missing Rejected', () => {
  ...
  it('Test 1: Rejects missing canonicalizationVersion', () => {
    const missingMatrix = JSON.parse(JSON.stringify(baseRemoteMatrix));
    delete missingMatrix.capabilityMatrixCanonicalizationVersion;
    
    expect(() => assertCapabilityDescriptorMatrixParity(missingMatrix))
      .toThrow('Capability matrix canonicalization version is missing');
  });
